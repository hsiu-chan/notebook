...
|>| Coronaviruses | &check;| ss linear RNA(+)| β-adrenergic, integrin
...
### 發育
- 先做 H chain, Ig &alpha;, &beta;
- 再來 &kappa; 失敗則 &lambda;
- 能做 IgM. IgD
- 布魯頓酪氨酸激酶 Bruton tyrosine kinase, BTK 

common lymphoid progenitor(CLP) $\rightarrow$ pro B $\xrightarrow{VDJ 重組}$ pre B $\xrightarrow{VJ 重組}$ immature B cell
...
### 發育
![alt text](paste_src/微免-109.png)


![alt text](paste_src/微免-110.png)
...
- Central memory T cell (T~CM~)
  - CCR7
  - 淋巴中心
- Effector memory T cell (T~EM~)
  - β1, β2(integrin)：
  - 周邊組織
  - 快速成熟，血液遷移
  - IFN-γ、IL-4、IL-5
- Tissue-resident memory T cell (T~RM~)
  - CD69 (長駐在組織)
...
m & d---->12[IL12]-->nk[NK]-->|"IFN-&gamma;"|m1[Macrophage]
12-->|"IFN-&gamma;"|th1[Th1]-->|"IFN-&gamma;"|m1


m & d ---->6[IL6]-->c[調理]-->ep[急性蛋白]
c-->t17[Th17, Tfh]

prr[PRRs]---> k -->1[IL-1]--->m1

m-->al["TNF&alpha;"]-->|聚集 TRAF|k["NF&kappa;"B]




d-->ia["IFN&alpha;"]-->nk[NK]

18[IL18]-->nk

inflammasome[inflammasome]---->1 & 18

1 & 6 & ia-->Li[Liver] --> cr[C-reactive protien]
...
## APC 
- 只有 [dendritic cell](#樹突狀細胞dendritic-cell) 能活化 T cell
- APC可以暴露在某些微生物的產物，增強抗原呈現的能力，這種東西稱為佐劑(adjuvants)。

### DCs
>CCL19, 21 吸引 CCR7
1. TLR signaling: 活化 DCs 表現 CCR7 
2. HEVs 表現 CCL21 吸引 DCs
3. DCs 進入 HEVs
4. CCL21 結合 CCR7 刺激 B7、增加 MHC
5. DCs 是一次性的
...
- Check point: allelc exclusion
  ...
  - T cell &alpha; chain 沒有 allelc exclusion

- Check point: light chain isotype exclusion
  - 先找 &kappa;，不行再去 &lambda;
  - &kappa;/&lambda; &sim; 1-3

- ==VpreB, &lambda;5== 代替輕鏈傳遞訊號

\picBox{
![alt text](paste_src/微免-118.png =60)
![alt text](paste_src/微免-119.png =30)}
...
![alt text](paste_src/微免-120.png =60)
...
## MHC 
- 有核就有 MHC I
- MHC II &alpha; chain 沒有多形性 (polymorphism)
- MHC I: 8-10 胺基酸
- MHC II 13-25 胺基酸
...
![alt text](paste_src/微免-106.png)

### MHC I (內源性抗原呈現)
1. MHC I &alpha; chain 在ER 製造，calnexin 幫助組裝
2. MHC I α:β2 形成， calnexin 放手
3. calreticulin, ERp57不讓MCH結合
4. Tapsin 幫助結合 TAP (transporter associated with antigen processing)
5. Proteasome 打爛 protein，餵給 MHC，TAP 審查肽鍊長度(8-16 &rarr; 8-10) 
6. 運輸到細胞膜
7. TCR 和 &alpha;3 結合
...
### MHC II (外源性抗原呈現)
1. protein 先被 endosome/lysosome 打爛
2. Li 在 ER 卡著 MHC 
2. 丟出 endosome，和 lysosome 結合
3. 酸性下LIP10水解，CLIP 繼續卡
5. HLA-DM 解開 CLIP 
6. TCR 結合 &beta;2
...
| **分化階段**        | **IL-2**              | naive T &rarr; effector T |
| ^        | ^              | B &rarr; Plasma  |
...
### IL-1 (&beta;)
- 活化單核吞噬細胞
- PRRs 的 [PNF-κB](#tlrs) 產生，可以互相活化
  - 所有免疫細胞甚至上皮都有 PRRs，處處能發炎
- [inflammasome](#細胞質)促進 IL-1β、IL-18 釋放 
...
- 分化T~FH~, T~H~17 ([Third signal](#cytokine-signals-third-signal))
- TNF-α, IL-1β, IL-6
...
|^|^|Dcs, T cell 連結|
|S1p|S1PR1| 淋巴 &rarr; 血管|

## CD? 

### CD1d
- [MHC class I-like](#mhc-i-like)
- 抗原呈現低抗原性的給 [NKT](#nkt)
...
![alt text](paste_src/微免-103.png =30)
...
- [血球滲出(diapedesis)](#血球滲出diapedesis)
...
### CD58 
- DCs 活化後表現黏附因子 ([APC](#apc))

### CD59
- [抑制 MAC (membrane attack complex)](#抑制)
...
- [穩定 MHC II](#mhc-ii-外源性抗原呈現)

### CD79A、B
- Igα 及Igβ


![alt text](paste_src/微免-116.png =30)
...
![alt text](paste_src/微免-122.png)


<embed src="mb.pdf" width="100%" height="600px" type="application/pdf">


## 怪提 

![alt text](paste_src/微免-121.png)
